...
<style>
  ...
  .container{
    display: grid;
    grid-template-columns: 1fr 20fr 1fr 3fr 2fr 2fr;
...
while($row = $result->fetch_assoc()) {
  ...
  echo '<input class="rowTx" id='.$k.' type="hidden" value="'.htmlspecialchars($row['questionText']).'">';
  echo '<input class="rowTy" id='.$k.' type="hidden" value="'.htmlspecialchars($row['questionType']).'">';

  $rowID=$row['ID'];
  $stmt2 = $conn->prepare("SELECT * FROM corrans WHERE questionID=?");
  $stmt2->bind_param("i", $rowID);
  $stmt2->execute();
  $result2 = $stmt2->get_result();
  $corrAns=[];
  while($row2 = $result2->fetch_assoc()) {
    array_push($corrAns, htmlspecialchars($row2['corrAns']).";");
  }
  $corrAns=implode($corrAns);
  echo '<input class="rowCA" id='.$k.' type="hidden" value="'.$corrAns.'">';
  $k=$k+1;
}
?>
